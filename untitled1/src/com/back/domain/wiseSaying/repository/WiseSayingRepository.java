...
//            System.out.println("저장한 마지막 번호 " + number);
    ...
    public void write(String content, String author) {
        try {
            int id = getLastIdFromFile()+1;
            File dir = new File("./db/wiseSaying");
            if (!dir.exists()) dir.mkdirs(); // 파일이 없을 경우 생성
            OutputStream output = new FileOutputStream("./db/wiseSaying/"+id+".json");
            String outputString = "{\n\t\"id\" : " + id + ",\n\t\"content\":\""+content+"\",\n\t\"author\":\""+author+"\"\n}";

            //DEBUG
            //System.out.println("저장하는 String: " + outputString);
            setLastIdForFile(id); //마지막 번호 파일 수정
    ...
    public void modify(int id, String content, String author) { //수정
        try {
            File dir = new File("./db/wiseSaying");
            if (!dir.exists()) dir.mkdirs(); // 파일이 없을 경우 생성
            OutputStream output = new FileOutputStream("./db/wiseSaying/"+id+".json");
